...
void main() {
  ...
  group('confirmation phrase', () {
    ...
    test('exact phrase is valid', () {
      expect(
        isTakeoutDuplicatesConfirmationPhraseValid(
  ...
  group('TakeoutDuplicateCleaner.run (async orchestration)', () {
    ...
    test('missing Google Fotos directory returns an all-zero summary', () async {
      final cleaner = TakeoutDuplicateCleaner(trashRoot: trashRoot);
    ...
    test('dry run reports wouldMove for a verified duplicate and touches '
      ...
      final outcome =
          summary.yearFolders.single.candidates.single;
    ...
    test('a size mismatch is skipped and never moved, even in confirm mode', () async {
      final canonical = File('$googleFotosDir/2024/IMG_0001.HEIC');
      final duplicate = File('$googleFotosDir/Fotos de 2024/IMG_0001.HEIC');
      await canonical.parent.create(recursive: true);
      await duplicate.parent.create(recursive: true);
    ...
    test(
      'a hash mismatch despite matching size is skipped and never moved — '
      'proves the three-way check is not weakened to size-only',
      () async {
        final canonical = File('$googleFotosDir/2024/IMG_0001.HEIC');
        final duplicate = File(
          '$googleFotosDir/Fotos de 2024/IMG_0001.HEIC',
        );
        await canonical.parent.create(recursive: true);
        await duplicate.parent.create(recursive: true);
    ...
    test(
      'a "Fotos de YYYY" folder with no matching "YYYY" canonical folder is '
      'reported and its files are never inspected',
      () async {
        final duplicate = File(
          '$googleFotosDir/Fotos de 2030/orphan_year.jpg',
        );
        await duplicate.parent.create(recursive: true);
    ...
    test(
      ...
      () async {
        ...
        final outcome = summary.yearFolders.single.candidates.single;
        expect(outcome.action, TakeoutDuplicateAction.skippedExisting);
        expect(
    ...
    test('preserves paths with spaces and unicode characters', () async {
      final canonical = File(
        '$googleFotosDir/2024/日本語 写真 vacaciones.jpg',
      );
      final duplicate = File(
  ...
  group(
    'Bash-vs-Dart parity: TakeoutDuplicateCleaner matches '
    ...
    () {
      test(
        ...
        'hash-mismatch-despite-matching-size case',
        () async {
          ...
          expect(
            bashStdout,
            contains('Kept canonical: ${canonical2024.path}'),
          );
          expect(
            bashStdout,
          ...
          final allCandidates = summary.yearFolders.expand(
            (y) => y.candidates,
          );
