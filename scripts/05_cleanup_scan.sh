...
for cmd in czkawka_cli ffmpeg ffprobe convert; do
  command -v "$cmd" >/dev/null || { echo "ERROR: $cmd not found. Run scripts/01_setup_dependencies.sh"; exit 1; }
done

[[ -d "$CLEANING_STAGING" ]] || { echo "ERROR: staging folder missing: $CLEANING_STAGING"; exit 1; }
...
echo "==> Summary"
ls -lh "$REPORT_DIR"
printf 'Image groups: '; grep -c '^Found .*images' "$REPORT_DIR/duplicate_images.txt" 2>/dev/null || true
printf 'Video groups: '; grep -c '^Found .*videos' "$REPORT_DIR/duplicate_videos.txt" 2>/dev/null || true
printf 'Exact duplicate groups: '; grep -c '^Found .*files' "$REPORT_DIR/duplicate_files.txt" 2>/dev/null || true
printf 'Video processing warnings: '; grep -c 'Failed to hash file' "$REPORT_DIR/video_scan_run.log" 2>/dev/null || true
